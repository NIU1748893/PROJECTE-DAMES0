#ifndef TAULER_HPP
#define TAULER_HPP

#include <string>
#include <vector>
#include <fstream>
#include "fitxa.hpp"
#include "moviment.hpp"
#include "posicio.hpp"
#include "const.hpp"

class Tauler
{
private:
    Fitxa m_caselles[N_FILES][N_COLUMNES];

public:
    Tauler();
    ~Tauler() = default;

    const Fitxa& getFitxa(const Posicio& pos) const;
    const Fitxa& getFitxa(int fila, int columna) const;
    bool esCasellaLliure(const Posicio& pos) const;
    void inicialitzaTaulerBuit();
    bool carregaPartidaDesArxiu(const string& nomArxiu);
    void colocaFitxa(const Fitxa& fitxa);

    void actualitzaMovimentsValids();
    void getPosicionsPossibles(const Posicio& origen, int& numPosicions, Posicio posicionsPossibles[]);
    bool mouFitxa(const Posicio& origen, const Posicio& desti);
    string toString() const;
    
    //valids¿?
    bool esPosicioValida(const Posicio& pos) const;
    bool esMovimentValid(const Posicio& origen, const Posicio& desti) const;

    void eliminaFitxa(const Posicio& pos);

    //regla buffar i obliga a capturar
    bool hiHaCapturaPossible(ColorFitxa color) const;
    Moviment obteMillorCaptura(ColorFitxa color) const;
    void aplicaReglaBufat(const Posicio& origenMoviment, const Moviment& movimentRealitzat, ColorFitxa colorActual);

private:
    //aux pa calcular moviments
    void calculaMovimentsFitxa(Fitxa& fitxa);
    void calculaMovimentsNormals(const Posicio& pos, ColorFitxa color, vector<Moviment>& moviments);
    void calculaMovimentsDama(const Posicio& pos, ColorFitxa color, vector<Moviment>& moviments);

    //gestio captures
    void cercaCaptures(const Posicio& pos, ColorFitxa color, TipusFitxa tipus, vector<Moviment>& moviments);
    void cercaCapturesRecursiu(Moviment& movimentActual, vector<Moviment>& moviments,
        vector<Posicio>& fitxesJaCapturades, ColorFitxa color, TipusFitxa tipus);
    bool potCapturar(const Posicio& posEnemic, const Posicio& posDesti, ColorFitxa color,
        const vector<Posicio>& fitxesJaCapturades) const;
    bool estaCapturada(const Posicio& pos, const vector<Posicio>& fitxesJaCapturades) const;

    //mov simple sense captures
    void cercaMovimentsSimples(const Posicio& pos, ColorFitxa color, TipusFitxa tipus, vector<Moviment>& moviments);

    void executaMoviment(const Moviment& moviment);
    void eliminaFitxesCapturades(const vector<Posicio>& captures);
    void promocionaFitxa(const Posicio& pos);

    //utilitis
    bool esDireccioValidaPerNormal(int deltaFila, ColorFitxa color) const;
    int comptaDamesCapturades(const Moviment& moviment) const;
};

#endif