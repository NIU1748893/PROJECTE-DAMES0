...
void Tauler::inicialitza(const string& nomFitxer) {
    ...
    while (getline(fitxer, linia)) {
        ...
        m_tauler[p.getFila()][p.getColumna()] = new Fitxa(tipus, color);
    }
}
        ...
        for (int dc = -2; dc <= 2; dc += 4) {
            ...
            if (nuevaFila >= 0 && nuevaFila < N_FILES && nuevaCol >= 0 && nuevaCol < N_COLUMNES) {
                if (m_tauler[nuevaFila][nuevaCol]->esBuida() &&
                    ...
                    m_tauler[filaIntermedia][colIntermedia]->getColor() != color) {
                    posicionsPossibles[nPosicions++] = Posicio(nuevaFila, nuevaCol);
                }
            }
        }
                ...
                for (int pasos = 1; pasos < N_FILES; pasos++) {
                    ...
                    if (m_tauler[nuevaFila][nuevaCol]->esBuida()) {
                        ...
                        if (m_tauler[nuevaFila][nuevaCol]->getColor() != color) {
                            ...
                            if (filaCaptura >= 0 && filaCaptura < N_FILES &&
                                colCaptura >= 0 && colCaptura < N_COLUMNES &&
                                m_tauler[filaCaptura][colCaptura]->esBuida()) {
...
void Tauler::visualitza() const {
    for (int fila = 0; fila < N_FILES; fila++) {
        for (int col = 0; col < N_COLUMNES; col++) {
            ...
            m_tauler[fila][col]->visualitza(posX, posY);
        }
    }
}
